...
@EXPORT = qw(
  ...
  NOERROR FORMERR SERVFAIL NXDOMAIN NOTIMP REFUSED DROP
  ...
  T_SRV T_ATMA T_NAPTR T_KX T_CERT T_A6 T_DNAME T_SINK T_OPT T_SPF T_TKEY T_TSIG
  ...
  rr_WKS rr_LOC rr_LOC_raw rr_SRV rr_AAAA rr_DNAME

  dns_answer dns_EDNS_answer dns_simple_dname dn_expand
...
sub NOTIMP()    { 4 }  # not implemented
sub REFUSED()   { 5 }  # query disallowed by policy

sub DROP()		{-10}	# Do not answer (useful for packet loss simulation)
...
sub rr_AAAA  { return pack('NNNN', shift,shift,shift,shift)        }
sub rr_CNAME { return dns_simple_dname(shift) }
sub rr_DNAME { return dns_simple_dname(shift) }
...
sub dns_EDNS_answer {
	my ($udp_payload_size,$extended_RCODE,$version,$do,$z,$data) = @_;
	my $serialized_data="";
	my $opt;
	if (defined $data)
	{
		foreach $opt (keys %$data)
		{
			$serialized_data.=pack('n',$opt).pack('n',length($$data{$opt})).$$data{$opt};
		}
	}
	return chr(0).pack('n2Nn',T_OPT(),$udp_payload_size,($extended_RCODE<<24)|($version<<16)|($do<<15)|$z,length($serialized_data)).$serialized_data;
}
...
B<Stanford::DNS> defines DNS name functions and constants.  The
B<dns_answer> function is perhaps the most important function.  It is
used to create DNS answers returned by B<Stanford::DNSserver> dynamic
...
B<Stanford::DNS> defines resource record functions for most DNS record
...
B<Stanford::DNS> defines many DNS constants.  The most commonly used
